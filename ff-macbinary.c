...

int
open_macbinary_file(char * filename, af_file_t * paf)
{
    int fd;
    int type = 0;
    size_t cFile = 0;
    struct MBData * pmbd = NULL;
    struct MBHeader * pmbh = NULL;
	...
	if (af_compute_crc(0, pBase, offsetof(MBHeader, u16_crc)) ==
	    af_read_u16(pmbh->u16_crc)) {
	    type = 2;
	    break;
	}
	...
	if (af_read_u32(pmbh->u32_data_length) > 0x007fffff ||
	    af_read_u32(pmbh->u32_resource_length) > 0x007fffff)
	    break;
    ...
    pmbd->iType = type;
    pmbd->cFile = cFile;
    pmbd->cData = af_read_u32(pmbh->u32_data_length);
    pmbd->cResource = af_read_u32(pmbh->u32_resource_length);
    pmbd->pBase = pBase;
    pmbd->pData = (char *)pBase + sizeof(*pmbh) +
	ROUND_UP(af_read_u16(pmbh->u16_expansion_len));
