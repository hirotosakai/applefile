...
af_ff_vtab afvtAppleSingleDouble = {
    close_file,
    resource_fork,
    data_fork,
};
...
static
int
open_appleheader_file(char * filename, af_file_t * paf, int iTypeRequired)
{
    int fd;
    unsigned int iEntry;
    size_t cData = 0, cEntries = 0, cFile = 0, cResource = 0;
    struct ASDData * pasdd;
    struct ASEntry * prase;
    struct ASHeader * pash;
    struct stat stat_buf;
    unsigned long id, type = 0, magicNum, magicRequired;
    void *pBase = (void *)-1, *pData = NULL, *pResource = NULL;
	...
	magicNum = af_read_u32(pash->u32_magicNum);
	if (magicNum != magicRequired)
	    break;

#if 0
	/* Disabled: macunpack uses 0x00000100, which has apparently
	   always been wrong, but still works */
	if (af_read_u32(pash->u32_versionNum) != 0x00020000)
	    break;
#endif
	cEntries = af_read_u16(pash->u16_numEntries);
	...
	for (iEntry = 0; iEntry < cEntries; iEntry++) {
	    id = af_read_u32(prase[iEntry].u32_entryID);

	    switch (id) {
		case AS_DATA:
		    pData = (char *)pBase +
			af_read_u32(prase[iEntry].u32_entryOffset);
		    cData = af_read_u32(prase[iEntry].u32_entryOffset);
		    break;
		case AS_RESOURCE:
		    pResource = (char *)pBase +
			af_read_u32(prase[iEntry].u32_entryOffset);
		    cResource = af_read_u32(prase[iEntry].u32_entryOffset);
		    break;
	    }
	}
...
int
open_applesingle_file(char * filename, af_file_t * paf)
{
    return open_appleheader_file(filename, paf, 1);
}

int
open_appledouble_file(char * filename, af_file_t * paf)
{
    char * pLastSlash;
	...
	r = open_appleheader_file(szHeaderFilename, paf, 2);
	if (r != 0)
	    break;
...
data_fork(af_file_t *paf, void **ppBase, size_t *pcSize)
{
	...
	*ppBase = pasdd->pDataFile;
	*pcSize = pasdd->cDataFile;
    }
